...
class EasyRuntime {
  ...
  readTags(tagsArr) {
    ...
    let promise = new Promise(function (resolve, reject) {
      ...
      self.get_rt().on('NotifyReadTag', (tagsobject, cookie) => {
        //register callback
  ...
  subscribeTags(tagsArr, cb_func) {
    ...
    self.get_rt().on('NotifySubscribeTag', (tagsobject, cookie) => {
      //register callback
      let req = cookieCb[cookie]
      if (req) {
        let resptags = fix_tags_order(req.tags, tagsobject)
    ...
    self.get_rt().on('NotifyUnsubscribeTag', cookie => {
      //register callback
      let req = cookieCb[cookie]
      if (req) {
        logger.error('Unsuscribed')
      }
    })
  ...
  writeTagValues(tagsValuesArr) {
    ...
    let promise = new Promise(function (resolve, reject) {
      ...
      self.get_rt().on('NotifyWriteTag', (tagsobject, cookie) => {
        //register callback
        let req = cookieCb[cookie]
        if (req) {
          req.fulfill()
  ...
  readAlarms(filter_string, options) {
    ...
    let promise = new Promise(function (resolve, reject) {
      ...
      self.get_rt().on('NotifyReadAlarm', (alarmsobject, cookie) => {
        //register callback
        let req = cookieCb[cookie]
        if (req) {
      ...
      self
        .get_rt()
        .ReadAlarm(self.rt_options.systems, filter_string, self.rt_options.language, newCookie) //read tag
    })
  ...
  subscribeAlarms(filter_string, options_or_cb_func, cb_func) {
    ...
    self.get_rt().on('NotifySubscribeAlarm', (alarmsobject, cookie) => {
      //register callback
      let req = cookieCb[cookie]
      if (req) {
        let respalarms = alarmsobject
    ...
    self.get_rt().on('NotifyUnsubscribeAlarm', cookie => {
      //register callback
      let req = cookieCb[cookie]
      if (req) {
        logger.error('Unsuscribed')
    ...
    self
      .get_rt()
      .SubscribeAlarm(self.rt_options.systems, filter_string, self.rt_options.language, newCookie) //subscribe alarm
